        ...
        public RemoteRepository(string server, int port)
        {
            _dataProvider = async expression =>
            {
                try
                {
                    ...
                    var response = await client.PostAsync("api/query", expression, new XmlMediaTypeFormatter());
                    ...
                    var result = await response.Content.ReadAsAsync<IEnumerable<DynamicObject>>();
                    return result;
                }
