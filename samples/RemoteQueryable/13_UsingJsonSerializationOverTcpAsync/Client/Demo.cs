...
namespace Client
{
    ...
    public class Demo
    {
        ...
        public async Task RunAsync()
        {
            ...
            foreach (Common.Model.Product i in await repo.Products.Where(p => idSelection.Contains(p.Id) || p.Id % 3 == 0 || p.Id == 10).ToListAsync())
            {
                Console.WriteLine($"  {i.Id} | {i.Name} | {i.Price:C}");
            }
            ...
            IQueryable<int> productIdsQuery =
                ...
                select p.Id;
            List<int> productIds = await productIdsQuery.ToListAsync();
