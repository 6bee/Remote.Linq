...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
        {
            ...
            var prod = repo.ProductGroups;
            ////from x in repo.ProductGroups
            ////select new
            ////{
            ////    x.Id,
            ////};
            var items = repo.OrderItems;
            ////from x in repo.OrderItems
            ////select new
            ////{
            ////    Id = x.Id - 10_000,
            ////    x.Quantity,
            ////};
            ////var m = prod.Max(p => p.Id);
            var q = items.Where(i => i.Id == prod.Max(p => p.Id));
            var r = await q.FirstOrDefaultAsync();
            ////var j = await (
            ////    from order in items
            ////    join detail in prod
            ////    on new { order.Id } equals new { detail.Id }
            ////    where order.Quantity == prod.Max(x => x.Id)
            ////    select order)
            ////    .FirstOrDefaultAsync();

            ////var s = await items.Select(o => new { o.Id, sum = items.Sum(oi => 1) }).FirstAsync();

            ////var result = await items.FirstOrDefaultAsync(o => o.Id == prod.Max(oi => oi.Id));
