...
namespace Remote.Linq.EntityFramework
{
    ...
    public class EntityFrameworkExpressionExecutor : ExpressionExecutor
    {
        ...
        private async Task<object> ExecuteAsync(System.Linq.Expressions.Expression expression, CancellationToken cancellationToken)
        {
            ...
            {
                if (ex.InnerException is InvalidOperationException invalidOperationException)
                {
                    if (string.Equals(invalidOperationException.Message, "Sequence contains no elements", StringComparison.Ordinal))
                    {
                        return new DynamicObject[0];
                    }

                    if (string.Equals(invalidOperationException.Message, "Sequence contains no matching element", StringComparison.Ordinal))
                    {
                        return new DynamicObject[0];
                    }

                    if (string.Equals(invalidOperationException.Message, "Sequence contains more than one element", StringComparison.Ordinal))
                    {
                        return new DynamicObject[2];
                    }

                    if (string.Equals(invalidOperationException.Message, "Sequence contains more than one matching element", StringComparison.Ordinal))
                    {
                        return new DynamicObject[2];
                    }
                }
