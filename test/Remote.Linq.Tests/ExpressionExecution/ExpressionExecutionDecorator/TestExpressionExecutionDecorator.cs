...
public class TestExpressionExecutionDecorator : ExpressionExecutionDecorator<DynamicObject>
{
    ...
    public TestExpressionExecutionDecorator(DefaultExpressionExecutor parent = null)
        : base(parent ?? new DefaultExpressionExecutor(null))
    {
    }
    ...
    public void AssertAllMethodsInvokedExacltyOnce(params int[] skip)
    {
        ...
        expectedCounters.ShouldAllBe(
            x => x.Count == 1,
            $"processor of {nameof(TestExpressionExecutionDecorator)} should be called since they are decorated rather then replaced by custom strategy");
        ...
        unexpectedCounters.ShouldAllBe(
            x => x.Count == 0,
            $"processor of {nameof(TestExpressionExecutionDecorator)} should be called since they are replaced rather then decorated with custom strategy");
    }
